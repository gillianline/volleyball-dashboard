...
st.markdown("""
    ...
    /* PDF PRINT LOGIC: EXPLICITLY HIDE CLUTTER */
    @media print {
        .stTabs [role="tablist"], 
        .no-print, 
...
try:
            ...
            with jc1:
                ...
                if not p_cmj_hist.empty:
                    ...
                    if latest['Test Date'] >= p_cmj_hist['Test Date'].max() - timedelta(days=10):
                        ...
                        label, color = ("ELITE", "#28a745") if p_diff > 0 else ("FATIGUED", "#dc3545")
    ...
    # --- TAB 2: MATCH V PRACTICE (SAME-DAY TREND FIX) ---
    with tabs[2]:
        ...
        if not w_data.empty and not g_data_l.empty:
            ...
            st.markdown('<div class="section-header">Weekly Load Trend (Connected Sessions)</div>', unsafe_allow_html=True)
    ...
    with tabs[4]:
        ...
        with c_ts2: pos_filter_t = st.selectbox("Filter by Position", ["All Positions"] + sorted(list(df['Position'].unique())), key="ms_pos_final")
        ...
        if selected_matches:
            ...
            if pos_filter_t != "All Positions": t_df = t_df[t_df['Position'] == pos_filter_t]
                        ...
                        with card_cols[j]:
                            ...
                            <div class="gallery-card">
                                <div style="display:flex; align-items:center; gap:15px; padding:15px; background:#f8f9fa; border-bottom:2px solid #FF8200;">
                                    ...
                                    <div>
                                        <p style="margin:0; font-weight:900; font-size:18px;">{name}</p>
            ...
            m_list = ['Total Jumps', 'Player Load', 'Estimated Distance (y)', 'Explosive Efforts']
            ...
            for idx, m in enumerate(m_list):
                with [r1, r2, r3, r4][idx]:
                    ...
                    fig.update_layout(title=f"Team Avg: {m}", template="simple_white", bargap=0.01); st.plotly_chart(fig, use_container_width=True, config=LOCKED_CONFIG)
