...
if check_password():
    # --- CSS: ULTIMATE LAYOUT & PRINT CONTROL ---
    st.markdown("""
        <style>
        ...
        /* PREVENT SPLITTING ATHLETES ON PRINT PAGES */
        .player-row {
        ...
        @media print {
            ...
            [data-testid="stMultiSelect"], [data-testid="stSelectbox"] { display: none !important; }
    ...
    try:
        ...
        with tabs[0]:
            ...
            with c_f1: selected_session = st.selectbox("Practice Selection", session_list, index=0)
            with c_f2: pos_f = st.selectbox("Position Filter", ["All Positions"] + sorted([p for p in df['Position'].unique() if p != "N/A"]))
            ...
            if not day_df.empty:
                ...
                score = math.ceil(t_grade / count) if count > 0 else 0
                ...
                with jc1:
                    p_hist = cmj_df[(cmj_df['Athlete'] == sel_p) & (cmj_df['Test Date'] <= curr_date)].sort_values('Test Date')
                    if not p_hist.empty:
                        latest = p_hist.iloc[-1]; base_h = p_hist.tail(5).iloc[:-1]['Jump Height (in)'].mean(); base_rsi = p_hist.tail(5).iloc[:-1]['RSI-modified [m/s]'].mean()
                ...
                with jc2:
                    if not p_hist.empty:
                        fig = make_subplots(specs=[[{"secondary_y": True}]]); fig.add_trace(go.Scatter(x=p_hist['Test Date'], y=p_hist['Jump Height (in)'], name="Height", line=dict(color='#FF8200', width=3)), secondary_y=False); fig.add_trace(go.Scatter(x=p_hist['Test Date'], y=p_hist['RSI-modified [m/s]'], name="RSI", line=dict(color='#4895DB', dash='dot')), secondary_y=True); fig.update_layout(height=280, margin=dict(l=0, r=0, t=20, b=0), showlegend=False); st.plotly_chart(fig, use_container_width=True, config=LOCKED_CONFIG)
                p_ph = phase_df[(phase_df['Name'] == sel_p) & (phase_df['Date'] == curr_date)]
                if not p_ph.empty:
                    st.markdown('<div class="section-header">Practice Phase Breakdown</div>', unsafe_allow_html=True)
                    fig_ph = go.Figure(); fig_ph.add_trace(go.Bar(x=p_ph['Phase'], y=p_ph['Total Jumps'], marker_color='#FF8200'))
                    fig_ph.update_layout(height=300); st.plotly_chart(fig_ph, use_container_width=True, config=LOCKED_CONFIG)
        ...
        with tabs[1]:
            ...
            for i in range(0, len(gal_df), 2):
                ...
                for j in range(2):
                    if i + j < len(gal_df):
                        pd_row = gal_df.iloc[i + j]; lb_g = df[(df['Name'] == pd_row['Name']) & (df['Date'] >= pd_row['Date'] - timedelta(days=30)) & (df['Date'] <= pd_row['Date'])]
                        r_html = ""; t_g = 0; c_m = 0
                        for k in all_metrics:
                            v, mx, avg = pd_row[k], lb_g[k].max(), lb_g[k].mean(); g = math.ceil((v / mx) * 100) if mx > 0 else 0
                            t_g += g; c_m += 1; diff = (v-avg)/avg if avg != 0 else 0; h = "class='bg-highlight-red'" if abs(diff) > 0.10 else ""; arr = f"<span class='arrow-red'>{'↑' if diff > 0.10 else '↓'}</span>" if abs(diff) > 0.10 else ""
                            r_html += f"<tr><td>{k}</td><td {h}>{v} {arr}</td><td>{g}</td></tr>"
                        sc = math.ceil(t_g / c_m)
                        with cols[j]: st.markdown(f'<div style="border:1px solid #E5E5E7; border-radius:15px; padding:15px; margin-bottom:20px;"><div style="display:flex; align-items:center; gap:10px;"><div style="flex:1.2; text-align:center;"><img src="{pd_row["PhotoURL"]}" class="gallery-photo"><p style="font-weight:bold;">{pd_row["Name"]}</p></div><div style="flex:3;"><table class="scout-table"><thead><tr><th>Metric</th><th>Val</th><th>Grade</th></tr></thead><tbody>{r_html}</tbody></table></div><div style="flex:1; text-align:center;"><div class="score-box" style="background-color:{get_flipped_gradient(sc)};">{sc}</div></div></div></div>', unsafe_allow_html=True)
        ...
        with tabs[2]:
            ...
            with c_ga: gp_p = st.selectbox("Athlete", sorted(df['Name'].unique()), key="gp_athlete")
            with c_gw:
                ...
                gp_w = st.selectbox("Week", w_r['L'].tolist()); sel_w = w_r[w_r['L'] == gp_w]['Week'].values[0]
            with c_gg: 
                game_opts = df[(df['Name'] == gp_p) & (df['Session_Type'] == 'Game') & (df['Week'] == sel_w)]['Session_Name'].unique()
                gp_g = st.selectbox("Select Specific Game", game_opts)
        ...
        with tabs[4]:
            ...
            st.markdown('<div id="selection-container">', unsafe_allow_html=True)
            ...
            sel_matches = st.multiselect("Select Weekend Matches", match_list_t, default=match_list_t[-3:] if len(match_list_t) >=3 else match_list_t)
            ...
            if sel_matches:
                ...
                m_map = {m: c_pal[idx % 3] for idx, m in enumerate(sel_matches)}
                st.markdown('<div class="section-header">Athlete Match Performance Breakdown</div>', unsafe_allow_html=True)
                tourney_df = df[df['Session_Name'].isin(sel_matches)]
                ...
                for name in sorted(tourney_df['Name'].unique()):
                    ...
                    # WRAPPING ENTIRE PLAYER BLOCK IN CONTAINER TO PREVENT PAGE BREAKS
                    ...
                    with sc1:
                        card = f"""
                            ...
                            <table class="scout-table"><thead><tr><th>Match</th><th>Jumps</th><th>Load</th><th>Effort</th></tr></thead><tbody>
                        """
                        for _, r in ad.iterrows():
                            card += f"<tr><td style='font-weight:700;'>{r['Session_Name']}</td><td>{int(r['Total Jumps'])}</td><td>{r['Player Load']:.0f}</td><td>{r['Explosive Efforts']:.0f}</td></tr>"
                        card += f"<tr style='background:#4895DB; color:white; font-weight:900;'><td>TOTAL</td><td>{int(ad['Total Jumps'].sum())}</td><td>{ad['Player Load'].sum():.0f}</td><td>{ad['Explosive Efforts'].sum():.0f}</td></tr></tbody></table>"
                ...
                c1, c2 = st.columns(2)
                with c1: st.plotly_chart(px.bar(team_avg, x='Session_Name', y='Total Jumps', title="Avg Total Jumps", color='Session_Name', color_discrete_map=m_map), use_container_width=True)
                with c2: st.plotly_chart(px.bar(team_avg, x='Session_Name', y='Player Load', title="Avg Player Load", color='Session_Name', color_discrete_map=m_map), use_container_width=True)
