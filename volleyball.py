...
if check_password():
    ...
    # --- ISOLATION STEP: Create master dataframes that can never be overwritten inside tab scopes ---
    df_master = raw_df[raw_df['Season'] == selected_season].copy()
    match_master = raw_match_df[raw_match_df['Season'] == selected_season].copy()
    cmj_master = cmj_df[cmj_df['Season'] == selected_season].copy()
    ash_master = ash_df[ash_df['Season'] == selected_season].copy()
    er_master = er_df[er_df['Season'] == selected_season].copy()
    phase_master = phase_df[phase_df['Season'] == selected_season].copy()
    ...
    with tabs[0]:
        ...
        with tab0:
            ...
            for s in session_list:
                s_date_series = df_master[df_master['Session_Name'] == s]['Date']
            ...
            with c_prof1:
                selected_session_prof = c_prof1.selectbox("Session Selection", clean_session_list_prof, index=0, key="nav_sel_prof_tab0")
            with c_prof2:
                selected_athlete_prof = c_prof2.selectbox("Athlete Selection", master_athlete_list, key="nav_ath_prof_tab0")

            if selected_session_prof == tournament_label:
                curr_date_prof = pd.to_datetime(target_date_str)
                p_session_data = df_master[(df_master['Name'] == selected_athlete_prof) & (df_master['Date'] == curr_date_prof)].copy()
                ...
                p_session_data = df_master[(df_master['Name'] == selected_athlete_prof) & (df_master['Session_Name'] == selected_session_prof)]
            ...
            if p_row.empty:
                curr_date_prof = pd.to_datetime(target_date_str) if selected_session_prof == tournament_label else pd.to_datetime(df_master['Date'].max() if not df_master.empty else "2026-01-01")
                meta_lookup = df_master[df_master['Name'] == selected_athlete_prof]
            ...
            p_full_prof = df_master[df_master['Name'] == selected_athlete_prof]
            ...
            p_cmj_hist = cmj_master[(cmj_master['Name'] == selected_athlete_prof) & (cmj_master['Test Date'] <= curr_date_prof)].sort_values('Test Date')
            cmj_col = 'Jump Height (Imp-Mom) [cm]'
            rsi_col = 'RSI-modified [m/s]'

            with jc1:
                baseline_cmj = p_cmj_hist[p_cmj_hist['Season'] == 'Summer'].head(1) if selected_season == 'Summer' else cmj_master[(cmj_master['Name'] == selected_athlete_prof) & (cmj_master['Week'] == 4)]
            ...
            with jc2:
                if not p_cmj_hist.empty:
                    ...
                    st.plotly_chart(fig, use_container_width=True, config=LOCKED_CONFIG, key="cmj_top_chart_isolated")
                else:
            ...
            p_ash_all = ash_master[(ash_master['Name'] == selected_athlete_prof) & (ash_master['Test Date'] <= curr_date_prof)].sort_values('Test Date')
            ...
            if not p_ash_all.empty:
                ...
                with ac2:
                    ...
                    if not p_ash_i_only.empty:
                        ...
                        st.plotly_chart(fig_ash, use_container_width=True, config=LOCKED_CONFIG, key="ash_profile_chart_isolated")
            else:
            ...
            p_er_hist = er_master[(er_master['Name'] == selected_athlete_prof) & (er_master['Test Date'] <= curr_date_prof)].sort_values('Test Date')
            ...
            if not p_er_hist.empty:
                ...
                with ec2:
                    ...
                    st.plotly_chart(fig_er, use_container_width=True, config=LOCKED_CONFIG, key="er_profile_chart_isolated")
            else:
    ...
    for idx, tab_name in enumerate(remaining_tabs, start=1):
        with tabs[idx]:
            ...
            with loop_container:
                ...
                # THE SCOPING FIX: Never use the parent "df" variable name inside loop components to ensure Tab 0 cannot leak data here
                if not df_master.empty:
                    # Enforce visualization rule: use Maximum position metrics instead of general averages
                    summary_view_loop = df_master.groupby('Position')[['Player Load', 'Total Jumps']].max().reset_index()
                    st.dataframe(summary_view_loop, use_container_width=True, hide_index=True, key=f"isolated_loop_df_key_{idx}")
                else:
