...
st.markdown("""
    ...
    .score-box { padding: 12px 20px; border-radius: 12px; font-size: 32px; font-weight: 800; min-width: 100px; color: #FFFFFF; }
    ...
    /* Info Box */
    .info-box { background-color: #f8f9fa; border-left: 5px solid #515154; padding: 10px; margin-top: 10px; font-size: 11px; color: #515154; }
    </style>
...
try:
    ...
    # --- MATTE COLOR SCALE (LESS NEON) ---
    def get_matte_gradient(score):
        score = float(score)
        if score >= 90: return "#4895DB" # Summitt Blue (Elite)
        if score >= 75: return "#FF8200" # Tennessee Orange (Good/High)
        if score >= 60: return "#8DAECF" # Muted Blue (Moderate)
        return "#515154" # Smokey Gray (Low)
    ...
    with tab_ind:
        if not day_df.empty:
            ...
            with c3:
                st.markdown(f'<div class="score-wrapper"><div class="score-label">Practice Score</div><div class="score-box" style="background-color:{get_matte_gradient(p["Practice Score"])};">{int(p["Practice Score"])}</div></div>', unsafe_allow_html=True)
    ...
    with tab_comp:
        ...
        fig_trend = px.line(practice_trends, x='Session_Name', y='Total Jumps', markers=True)
        fig_trend.update_traces(line_color='#FF8200', line_width=4); fig_trend.update_layout(height=350, xaxis_title=None, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)'); st.plotly_chart(fig_trend, use_container_width=True)
    ...
    with tab_gp:
        ...
        with c_gp_ath: gp_ath_sel = st.selectbox("Select Athlete", df['Name'].unique(), key="gp_ath")
        with c_gp_week:
            if 'Week' in df.columns:
                ...
                week_options = dict(zip(week_ranges['Label'], week_ranges['Week'])); gp_label_sel = st.selectbox("Select Week", list(week_options.keys()), key="gp_week"); gp_week_id = week_options[gp_label_sel]
        with c_gp_game: ath_games = df[(df['Name'] == gp_ath_sel) & (df['Session_Type'] == 'Game')]['Session_Name'].unique(); gp_game_target = st.selectbox("Select Target Game", ath_games, key="gp_game")
        ...
        if 'Week' in df.columns:
            ...
            if not week_data.empty:
                ...
                with cg1:
                    st.write(f"**{gp_week_id} Prep vs. Game Demand**")
                    for m in crit_mets:
                        g_val = game_data[m]; w_val = week_avg[m]; pct = (w_val / g_val * 100) if g_val > 0 else 0
                        st.metric(label=m, value=f"{w_val:.1f}", delta=f"{pct:.1f}% of Game Demand", delta_color="inverse" if pct > 100 else "normal")
                with cg2:
