...
if check_password():
    ...
    st.markdown("""
        ...
        @media print {
            /* Keep Title and Logo visible */
            .main-logo-container { display: block !important; margin-bottom: 0 !important; }
            
            /* Hide Tab Bar, Sidebar, and Selection Area */
            .stTabs [role="tablist"], [data-testid="stSidebar"], header, footer, button, #print-selector-block { 
            ...
            /* Force the Data area to show */
            #print-report-content, .player-row-container { display: block !important; opacity: 1 !important; visibility: visible !important; }
    ...
    try:
        ...
        # Shared Global Filters (prevents undefined name errors)
        c_nav1, c_nav2 = st.columns(2)
        with c_nav1: selected_session = st.selectbox("Global Selection", session_list, index=0)
        with c_nav2: pos_f = st.selectbox("Global Filter", ["All Positions"] + sorted([p for p in df['Position'].unique() if p != "N/A"]))
        ...
        with tabs[2]:
            ...
            with cga: gpp = st.selectbox("Athlete Selection", sorted(df['Name'].unique()), key="gp_a_vf")
            with cgw:
                ...
                gpw = st.selectbox("Week Selection", wr['L'].tolist(), key="gp_w_vf"); sel_w = wr[wr['L'] == gpw]['Week'].values[0]
            with cgg:
                go_opts = df[(df['Name'] == gpp) & (df['Session_Type'] == 'Game') & (df['Week'] == sel_w)]['Session_Name'].unique()
                gpg = st.selectbox("Select Specific Game", go_opts, key="gp_g_vf")
        ...
        with tabs[4]:
            ...
            ms_pos = st.selectbox("Filter by Position", ["All Positions"] + sorted(list(df['Position'].unique())), key="ms_surgical")
            st.markdown('</div>', unsafe_allow_html=True)
            
            # This container explicitly stays visible in Print via CSS
            st.markdown('<div id="print-report-content">', unsafe_allow_html=True)
            if ms_matches:
                ...
                for name in sorted(tdf['Name'].unique()):
                    ...
                    with sc2:
                        ...
                        fig.update_layout(barmode='group', height=260, margin=dict(l=10, r=10, t=10, b=80), template="simple_white", font=dict(color="#333333", size=10), legend=dict(orientation="h", yanchor="top", y=-0.3, xanchor="center", x=0.5))
                ...
                with tc1: st.plotly_chart(px.bar(tavg, x='Session_Name', y='Total Jumps', title="Avg Jumps", color='Session_Name', color_discrete_map=mmap), use_container_width=True, config=LOCKED_CONFIG)
                with tc2: st.plotly_chart(px.bar(tavg, x='Session_Name', y='Player Load', title="Avg Load", color='Session_Name', color_discrete_map=mmap), use_container_width=True, config=LOCKED_CONFIG)
            st.markdown('</div>', unsafe_allow_html=True)
