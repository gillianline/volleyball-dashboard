...
st.markdown("""
    ...
    .score-box { padding: 20px; border-radius: 12px; font-size: 40px; font-weight: 800; text-align: center; color: #1D1D1F; }
...
try:
    ...
    # Only show Comparison Date on Profile and Comparison tabs
    active_tab = st.session_state.get('active_tab', "Session Flow")
    ...
    date_b_str = "None"
    # Logic to only show comparison dropdown if relevant
    with col_d2:
    ...
    day_df = day_df.apply(lambda x: process_player(x, df), axis=1).sort_values('Name')
    ...
    with t_player:
        ...
        with c2:
            ...
            if date_b_str != "None":
                ...
                if not p_b.empty:
                    p_b = process_player(p_b.iloc[0], df)
                    for k in grading_map.keys():
        ...
        # ADDED INFO: Individual Phase Breakdown
        st.divider()
        ...
        if not p_phases.empty:
            fig_ind = px.bar(p_phases, x="Phase", y="Total Player Load", template="plotly_white", color_discrete_sequence=["#FF8200"])
    ...
    with t_comp:
        if date_b_str != "None":
            df_b = df[df['Date'] == pd.to_datetime(date_b_str)].copy()
            ...
            st.plotly_chart(px.bar(pd.DataFrame([r for r in comp_rows if "Distance" not in r["Metric"]]).melt(id_vars="Metric", value_vars=[date_a_str, date_b_str]), x="Metric", y="value", color="variable", barmode="group", template="plotly_white", color_discrete_sequence=["#FF8200", "#545454"]), use_container_width=True)
        else:
            st.info("Select a comparison date at the top to unlock team trends.")
