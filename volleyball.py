...
# Tennessee Branding: #FF8200 (Orange) & Neutral Whites/Grays
st.markdown("""
    <style>
    .stApp { background-color: #FFFFFF; color: #1D1D1F; }
    
    /* Center all table content, No Decimals, No Index */
    .scout-table {
        width: 100%;
    ...
    .player-photo-large {
        ...
        height: 240px;
        object-fit: cover;
        border: 6px solid #FF8200; /* Tennessee Orange */
...
try:
    ...
    # Date Picker in the Main Area
    st.markdown("<h3 style='text-align: center;'>Select Practice Session</h3>", unsafe_allow_html=True)
    ...
    # --- APPLY LOGIC & SET CONSISTENT ORDER ---
    day_df = day_df.apply(process_player, axis=1).sort_values('Name') # Alphabetical Order Locked
    
    def render_custom_table(dataframe, cols_to_show):
        html = '<table class="scout-table"><thead><tr>'
        for col in cols_to_show: html += f'<th>{col}</th>'
        html += '</tr></thead><tbody>'
    ...
    with tab3:
        # Team Gallery: 2 per row, Alphabetical order
            ...
            for j in range(2):
                if i + j < len(day_df):
                    ...
                    with cols[j]:
                        ...
                        with ct:
                            c_rows = [{"Metric": grading_map[k], "Current": p_d[k], "Max": p_d[f'{k}_Max'], "Grade": p_d[f'{k}_Grade']} for k in grading_map.keys()]
                            st.markdown(render_custom_table(pd.DataFrame(c_rows), ["Metric", "Current", "Max", "Grade"]), unsafe_allow_html=True)
                        with cs:
                            st.markdown(f'<div style="background-color:{p_score_bg}; color:#1D1D1F; border-radius:10px; text-align:center; padding:15px; font-size:26px; font-weight:800; margin-top:45px;">{int(p_d["Practice Score"])}</div>', unsafe_allow_html=True)
                        st.markdown('</div>', unsafe_allow_html=True)

    with tab4:
        st.subheader("Leaderboard")
        # Fixed Leaderboard: Now correctly pulls filtered day data and hides index
