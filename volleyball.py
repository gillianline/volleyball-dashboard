...
def load_all_data():
    ...
    # Add calculated columns matching Excel ForceDecks Data table
    ...
    # Relative forces if not provided
...
if check_password():
    ...
    try:
        ...
        # Exact Column Naming from ForceDecks Data
        ...
        if selected_season == "Testing":
            ...
            if selected_testing_tab == "CMJ Dashboard":
                ...
                # --- EXACT EXCEL FORMULA ENGINE: READINESS / WELLNESS SCORE (References Sheet U8:U15) ---
                # Excel Formulas:
                # mRSI:              =IF(100+(((T8-S8)/S8)*100)>100, 100, 100+(((T8-S8)/S8)*100))
                # ECC RFD:           =IF(100+(((T9-S9)/S9)*100)>100, 100, 100+(((T9-S9)/S9)*100))
                # Force @ 0 Velo:    =IF(100+(((T10-S10)/S10)*100)>100, 100, 100+(((T10-S10)/S10)*100))
                # TTO (Inverted):    =IF(100-(((T11-S11)/S11)*100)>100, 100, 100-(((T11-S11)/S11)*100))
                # ECC Peak Velo:     =IF(100+(((T12-S12)/S12)*100)>100, 100, 100+(((T12-S12)/S12)*100))
                # Ecc Peak Power:    =IF(100+(((T13-S13)/S13)*100)>100, 100, 100+(((T13-S13)/S13)*100))
                # P2:P1 Con Imp (Inv): =IF(100-(((T14-S14)/S14)*100)>100, 100, 100-(((T14-S14)/S14)*100))
                # Overall Score:     =AVERAGE(U8:U14)
                ...
                if sel_cmj_mode == "Individual Athlete":
                    ...
                    if ath_cmj_all.empty:
                        ...
                        # Previous Trial Row (Trial - 1, matching References L3 = L4 - 1)
                        cur_pos = list(ath_cmj_all.index).index(cur_idx_list[-1])
                        prev_test_row = ath_cmj_all.iloc[max(0, cur_pos - 1)]

                        # Baseline Row (matching Dashboard T6:T14)
                        baseline_candidates = ath_cmj_all[ath_cmj_all['Trial Number'].astype(str).str.lower() == 'baseline'] if 'Trial Number' in ath_cmj_all.columns else pd.DataFrame()
                        base_test_row = baseline_candidates.iloc[0] if not baseline_candidates.empty else ath_cmj_all.iloc[0]
                        ...
                        # Exact 9 Dashboard Performance Table Metrics (Dashboard O6:O14)
                        cmj_metric_defs = [
                        ...
                        # Calculate Final Excel Readiness Score
                        ...
                        # Data Table (Replicating Dashboard O5:Z14)
                        with top_col2:
                            table_rows_str = ""
                            for m_info in cmj_metric_defs:
                                ...
                                # Dashboard Z6:Z14 = ((Current - Baseline) / Baseline)
                        ...
                        # --- EXACT EXCEL FORMULA ENGINE: T-SCORE STANDARDS (References Sheet E8:J65) ---
                        # 10 Standards Metrics mapped directly to References rows:
                        # Jump Height (E8), Jump Momentum (E14), Peak Velocity (E20), Mean Con Force (E26),
                        # Force @ 0 Velocity (E32), Positive Impulse (E38), P1 Con Impulse (E44), P2 Con Impulse (E50),
                        # Adjusted CMD (E56), Time to Takeoff (E62 - inverted z-score)
                        ...
                        with chart_col:
                            ...
                            # Filter reference pool by Comparison Factor
                            ...
                            for bm in bar_metrics_excel:
                                ...
                                if cname in ref_pool_df.columns and len(ref_pool_df[cname].dropna()) > 1:
                                    ...
                                    m_std = valid_series.std(ddof=1) # Excel STDEV uses sample standard deviation (ddof=1)
                                    
                                    if m_std > 0:
                                        # References H8 = STANDARDIZE(F8, F10, F11); References H62 = STANDARDIZE(...) * -1
                                        if bm["invert"]:
                                            z_val = -1.0 * ((ath_v - m_mean) / m_std)
                                        else:
                                            z_val = (ath_v - m_mean) / m_std
                                        
                                        # References J8 = 50 + H8 * 10
                ...
                # --- SUB-TAB 2: TEAM CMJ SUMMARY & READINESS DASHBOARD ---
                    ...
                    for ath_name in sorted(raw_cmj_df['Name'].unique()):
                        ...
                        # Find previous trial
                        cur_idx_pos = list(ath_sub_cmj.index).index(target_row.name)
                        prev_row = ath_sub_cmj.iloc[max(0, cur_idx_pos - 1)]
        ...
        # ==========================================
        # --- SEASONAL GPS VIEWS (Spring/Summer/Pre-Season)
        # ==========================================
        else:
            ...
            # --- SUB-VIEWS FOR SEASONAL TRACKING ---
                    ...
                    if raw_cmj_df is not None and not raw_cmj_df.empty:
                        ...
                        if not baseline_cmj.empty:
                            ...
                            if latest_post is not None:
                                ...
                                m1.metric("Baseline", f"{base_row[cmj_col]:.1f}")
                                m2.metric("Latest Jump", f"{latest_post[cmj_col]:.1f}", f"{((latest_post[cmj_col] - base_row[cmj_col]) / base_row[cmj_col]) * 100:+.1f}%")
                            ...
                            for _, row in post_match_cmj.iterrows():
                                ...
                                comparison_list.append({"Date": jump_date.strftime('%m/%d/%Y'), "Prev Match": prev_match_name, "Jump Height": f"{row[cmj_col]:.1f}", "Raw Diff": raw_diff, "Display Diff": f"{raw_diff:+.1f}", "RSI": f"{row[rsi_col]:.2f}"})
                            ...
                            fig_cmj.add_trace(go.Scatter(x=ath_cmj_data['Test Date'], y=ath_cmj_data[cmj_col], name="Jump Height", mode='lines+markers', line=dict(color='#4895DB', width=3)), secondary_y=False)
