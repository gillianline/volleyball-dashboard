...
try:
            ...
            for i in range(0, len(gal_df), 2):
                ...
                for j in range(2):
                    if i + j < len(gal_df):
                        ...
                        for k in ['Total Jumps', 'Player Load', 'Estimated Distance', 'Explosive Efforts']:
                            v, m = pd_row[k], lb_g[k].max(); g = math.ceil((v / m) * 100) if m > 0 else 0; t_grade += g; c_metrics += 1; r_html += f"<tr><td>{k}</td><td>{v}</td><td>{m}</td><td>{g}</td></tr>"
                        sc_g = math.ceil(t_grade / c_metrics) if c_metrics > 0 else 0
                        with cols[j]: st.markdown(f'<div class="gallery-card" style="min-height:300px;"><div style="display:flex; align-items:center; gap:10px;"><div style="flex:1.2; text-align:center;"><img src="{pd_row["PhotoURL"]}" class="gallery-photo" style="width:80px; height:80px;"><p style="font-weight:bold; font-size:14px; margin-top:8px;">{pd_row["Name"]}</p></div><div style="flex:3;"><table class="scout-table"><thead><tr><th>Metric</th><th>Val</th><th>Max</th><th>Grade</th></tr></thead><tbody>{r_html}</tbody></table></div><div style="flex:1; text-align:center;"><div style="background-color:{get_flipped_gradient(sc_g)}; color:white; padding:8px; border-radius:12px; font-size:24px; font-weight:900;">{sc_g}</div></div></div></div>', unsafe_allow_html=True)

    # --- TAB 2: GAME V PRACTICE (FIXED GHOST BAR) ---
    with tabs[2]:
        ...
        if not w_data.empty and not g_data_l.empty:
            low_m = ['Total Jumps', 'Player Load', 'Explosive Efforts']; w_avg = w_data[low_m + ['Estimated Distance']].mean(); g_d = g_data_l.iloc[0]; cg1, cg2 = st.columns([1, 2])
            with cg1:
                for m in low_m + ['Estimated Distance']: pdif = ((w_avg[m] - g_d[m]) / g_d[m] * 100) if g_d[m] > 0 else 0; st.metric(label=m, value=f"{g_d[m]:.0f}", delta=f"{pdif:+.1f}% vs Weekly Avg")
            with cg2:
                fig_dual = make_subplots(specs=[[{"secondary_y": True}]]); fig_dual.add_trace(go.Bar(x=low_m, y=[w_avg[m] for m in low_m], name="Weekly Avg", marker_color='#4895DB'), secondary_y=False); fig_dual.add_trace(go.Bar(x=low_m, y=[g_d[m] for m in low_m], name=f"Game Output", marker_color='#FF8200'), secondary_y=False); fig_dual.add_trace(go.Bar(x=['Estimated Distance'], y=[w_avg['Estimated Distance']], name="Weekly Distance", marker_color='#4895DB', opacity=0.3), secondary_y=True); fig_dual.add_trace(go.Bar(x=['Estimated Distance'], y=[g_d['Estimated Distance']], name="Game Distance", marker_color='#FF8200', opacity=0.3), secondary_y=True); fig_dual.update_layout(height=400, barmode='group', showlegend=True, legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1)); st.plotly_chart(fig_dual, use_container_width=True, config=LOCKED_CONFIG)
    ...
    # --- TAB 4: TOURNAMENT SUMMARY (FIXED BARS & GHOST DISTANCE) ---
    with tabs[4]:
        ...
        with st.container():
            ...
            with c_ts2: pos_filter_t = st.selectbox("Filter by Position", ["All Positions"] + sorted(list(df['Position'].unique())), key="tourney_pos_filter")
        if selected_games:
            color_palette = ['#4895DB', '#FF8200', '#515154']; game_color_map = {game: color_palette[i % len(color_palette)] for i, game in enumerate(selected_games)}
            ...
            for i in range(0, len(athletes_t), 2):
                ...
                for j in range(2):
                    if i + j < len(athletes_t):
                        ...
                        with card_cols[j]:
                            card_html = f'<div class="gallery-card"><div style="display:flex; align-items:center; gap:15px; padding:15px; background:#f8f9fa; border-bottom:2px solid #FF8200;"><img src="{ath_data_t["PhotoURL"].iloc[0]}" class="gallery-photo"><div><p style="margin:0; font-weight:900; color:#1D1D1F; font-size:18px;">{ath_name_t}</p><p style="margin:0; color:#4895DB; font-weight:700; font-size:12px;">{ath_data_t["Position"].iloc[0]}</p></div></div><div style="padding:10px 15px;"><table class="scout-table" style="margin-bottom:0;"><thead><tr><th>Match</th><th>Total Jumps</th><th>Player Load</th><th>Estimated Distance</th><th>Explosive Efforts</th></tr></thead><tbody>'
                            ...
                            for _, r in ath_data_t.iterrows():
                                ...
                                fig_ath.add_trace(go.Bar(name=f"Distance", x=['Estimated Distance'], y=[r['Estimated Distance']], marker_color=game_color_map[r['Session_Name']], opacity=0.3, showlegend=False), secondary_y=True)
                            fig_ath.update_layout(barmode='group', height=320, margin=dict(l=40,r=40,t=10,b=20), legend=dict(orientation="h", yanchor="top", y=-0.2, xanchor="center", x=0.5), template="simple_white")
                            fig_ath.update_yaxes(title_text="Jumps/Load/Efforts", secondary_y=False); fig_ath.update_yaxes(title_text="Estimated Distance", secondary_y=True); st.plotly_chart(fig_ath, use_container_width=True, config=LOCKED_CONFIG); st.markdown("</div>", unsafe_allow_html=True)
            ...
            t_m_list = ['Total Jumps', 'Player Load', 'Estimated Distance', 'Explosive Efforts']; team_avg_t = df[df['Session_Name'].isin(selected_games)].groupby(['Session_Name', 'Sheet_Order'])[t_m_list].mean().reset_index().sort_values('Sheet_Order')
            r1c1, r1c2 = st.columns(2); r2c1, r2c2 = st.columns(2); locs = [r1c1, r1c2, r2c1, r2c2]
            for idx, m in enumerate(t_m_list):
                with locs[idx]:
                    fig_t = go.Figure(); fig_t.add_trace(go.Bar(x=team_avg_t['Session_Name'], y=team_avg_t[m], marker_color=[game_color_map[g] for g in team_avg_t['Session_Name']], marker_line_width=0))
