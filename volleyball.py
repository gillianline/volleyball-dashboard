...
try:
    ...
    with tabs[0]:
        ...
        if not day_df.empty:
            ...
            score = math.ceil(total_grade / count); c1, c2, c3 = st.columns([1.2, 2.5, 1.2])
            ...
            with jc1:
                ...
                if not p_cmj_hist.empty:
                    ...
                    label, color = ("ELITE", "#28a745") if latest['Jump Height (in)'] >= base_h and latest['RSI-modified [m/s]'] >= base_rsi else ("FATIGUED", "#dc3545")
            ...
            for i in range(0, len(gal_df), 2):
                ...
                for j in range(2):
                    if i + j < len(gal_df):
                        ...
                        sc_g = math.ceil(t_grade / c_metrics); with cols[j]: st.markdown(f'<div class="gallery-card" style="min-height:300px;"><div style="display:flex; align-items:center; gap:10px;"><div style="flex:1.2; text-align:center;"><img src="{pd_row["PhotoURL"]}" class="gallery-photo" style="width:80px; height:80px;"><p style="font-weight:bold; font-size:14px; margin-top:8px;">{pd_row["Name"]}</p></div><div style="flex:3;"><table class="scout-table"><thead><tr><th>Metric</th><th>Val</th><th>Max</th><th>Grade</th></tr></thead><tbody>{r_html}</tbody></table></div><div style="flex:1; text-align:center;"><div style="background-color:{get_flipped_gradient(sc_g)}; color:white; padding:8px; border-radius:12px; font-size:24px; font-weight:900;">{sc_g}</div></div></div></div>', unsafe_allow_html=True)

    # --- TAB 2: GAME V PRACTICE (PRINT-SAFE DUAL AXIS) ---
    with tabs[2]:
        ...
        if not w_data.empty and not g_data_l.empty:
            ...
            with cg2:
                fig_v = make_subplots(specs=[[{"secondary_y": True}]]); fig_v.add_trace(go.Bar(x=low_m, y=[w_avg[m] for m in low_m], name="Weekly Avg", marker_color='#4895DB'), secondary_y=False); fig_v.add_trace(go.Bar(x=low_m, y=[g_d[m] for m in low_m], name=f"Game Output", marker_color='#FF8200'), secondary_y=False)
                # PRINT SAFE DISTANCE: Opacity 0.6 + solid border
                fig_v.add_trace(go.Bar(x=['Estimated Distance'], y=[w_avg['Estimated Distance']], name="Weekly Distance", marker=dict(color='#4895DB', opacity=0.6, line=dict(color='#4895DB', width=2))), secondary_y=True); fig_v.add_trace(go.Bar(x=['Estimated Distance'], y=[g_d['Estimated Distance']], name="Game Distance", marker=dict(color='#FF8200', opacity=0.6, line=dict(color='#FF8200', width=2))), secondary_y=True)
                fig_v.update_layout(height=400, barmode='group', showlegend=True); st.plotly_chart(fig_v, use_container_width=True, config=LOCKED_CONFIG)
    ...
    # --- TAB 4: TOURNAMENT SUMMARY (PRINT SAFE) ---
    with tabs[4]:
