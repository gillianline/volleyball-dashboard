...
# --- INITIAL PASS DESIGN OVERRIDES ---
st.markdown("""
    <style>
    th, td {text-align: center !important;}
    [data-testid="stMetricValue"] {font-size: 24px;}
    div[data-testid="stMetric"] { background-color: #F8F9FA; padding: 10px; border-radius: 8px; border-left: 4px solid #FF8200; }
    </style>
...
if check_password():
    ...
    with tabs[0]:
        ...
        # FIXING THE BLEED: Construct native dataframe items for clean grid output instead of printing raw strings
        summary_rows = []
        t_grade_prof = 0
        c_metrics_prof = 0
        ...
        for k in filtered_metrics_prof:
            ...
            status_arrow = "↑" if diff > 0.10 else "↓" if diff < -0.10 else "—"
            summary_rows.append({"Metric": k, "Today Total": f"{val:.1f}", "30d Max Day": f"{mx:.1f}", "Trend": status_arrow, "Grade %": f"{g}%"})

        sc_prof = math.ceil(t_grade_prof / c_metrics_prof) if c_metrics_prof > 0 else 0

        c1, c2, c3 = st.columns([1.2, 2.5, 1.2])
        with c1:
            st.image(p_meta.get("PhotoURL", "https://www.w3schools.com/howto/img_avatar.png"), width=180)
            st.subheader(p_meta.get("Name", selected_athlete_prof))
            st.caption(f"Position context: {p_meta.get('Position', 'N/A')}")
        with c2:
            st.dataframe(pd.DataFrame(summary_rows), use_container_width=True, hide_index=True)
        with c3:
            st.metric(label="SESSION EFFORT SCORE", value=f"{sc_prof} / 100", delta=f"{sc_prof - 50} vs Baseline")
        
        st.subheader("Weekly Readiness Profile")
        
        # --- BLOCK 1: LOWER BODY JUMP PROFILE (CMJ) ---
        st.markdown("**COUNTERMOVEMENT JUMP (CMJ)**")
        jc1, jc2 = st.columns([2, 3])
        ...
        with jc1:
            ...
            if not baseline_cmj.empty and not p_cmj_hist.empty:
                ...
                sub_c1.metric("CMJ Height", f"{cur_h:.1f} cm", f"{p_diff_h:+.1f}% vs Base")
                sub_c2.metric("RSI Modified", f"{cur_rsi:.2f}", f"{p_diff_rsi:+.1f}% vs Base")
                st.caption(f"Target Baselines: CMJ: {base_h:.1f} cm | RSI: {base_rsi:.2f}")
            else:
                st.warning("No jump data recorded.")
        ...
        st.markdown("**ASH SHOULDER CAPACITY (ISO I)**")
        ...
        if not p_ash_all.empty:
            ...
            with ac1:
                ...
                sub_ac1.metric("Left Shoulder Force", f"{li:.0f} N", f"{pct_l:+.1f}% vs Base")
                sub_ac2.metric("Right Shoulder Force", f"{ri:.0f} N", f"{pct_r:+.1f}% vs Base")
                st.caption(f"Measured Asymmetry Index: {asym_i:+.1f}% (Base: L {base_li:.0f}N / R {base_ri:.0f}N)")
            with ac2:
            ...
            st.info("No shoulder testing profiles verified for timeline.")

        # --- BLOCK 3: ROTATOR CUFF RANGE OF MOTION ---
        st.markdown("**ROTATOR CUFF INTEGRITY (EXTERNAL ROTATION ROM)**")
        ...
        if not p_er_hist.empty:
            ...
            with ec1:
                ...
                sub_ec1.metric("Left ROM (°)", f"{cur_l_rom:.1f}°")
                sub_ec2.metric("Right ROM (°)", f"{cur_r_rom:.1f}°")
                st.caption(f"Measured Mobility Asymmetry: {cur_asym_rom:+.1f}%")
            with ec2:
                fig_er = go.Figure()
    ...
    # --- HISTORICAL ANALYSIS REBOOTS FOR REMAINING WORKSPACES ---
    for idx in range(1, 11):
        with tabs[idx]:
            st.subheader("Positional Performance Summary Matrix")
            if not df.empty:
                # Setting visualization rule explicitly to Maximum values per custom operational standard
                summary_view = df.groupby('Position')[['Player Load', 'Total Jumps']].max().reset_index()
                st.dataframe(summary_view, use_container_width=True, hide_index=True)
            else:
                st.info("Awaiting structural dataset streaming logs.")
