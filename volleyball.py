...
try:
    ...
    with tabs[0]:
        ...
        if not day_df.empty:
            ...
            if not p_ph.empty:
                ...
                fig_ph.update_layout(height=350, showlegend=True)
                ...
                for _, r in p_ph.iterrows(): 
                    p_tbl += f"<tr><td>{r['Phase']}</td><td>{int(r['Total Jumps'])}</td><td>{r['Player Load']:.1f}</td></tr>"
    ...
    with tabs[2]:
        ...
        if not w_data.empty and not g_data_l.empty:
            low_m = ['Total Jumps', 'Player Load', 'Explosive Efforts']
            w_avg = w_data[low_m + ['Estimated Distance']].mean()
            g_d = g_data_l.iloc[0]
            cg1, cg2 = st.columns([1, 2])
            with cg1:
                for m in low_m + ['Estimated Distance']: 
            ...
            with cg2:
                ...
                fig_v.add_trace(go.Bar(x=low_m, y=[w_avg[m] for m in low_m], name="Weekly Avg", marker_color='#4895DB'), secondary_y=False)
                fig_v.add_trace(go.Bar(x=low_m, y=[g_d[m] for m in low_m], name=f"Game Output", marker_color='#FF8200'), secondary_y=False)
                fig_v.add_trace(go.Bar(x=['Estimated Distance'], y=[w_avg['Estimated Distance']], name="Weekly Distance", marker=dict(color='#4895DB', opacity=0.6, line=dict(color='#4895DB', width=2))), secondary_y=True)
                fig_v.add_trace(go.Bar(x=['Estimated Distance'], y=[g_d['Estimated Distance']], name="Game Distance", marker=dict(color='#FF8200', opacity=0.6, line=dict(color='#FF8200', width=2))), secondary_y=True)
                fig_v.update_layout(height=400, barmode='group', showlegend=True)
    ...
    with tabs[3]:
        ...
        tr_metrics = ["Player Load", "Estimated Distance", "Total Jumps"]
        ...
        for i, m in enumerate(tr_metrics):
            with cols[i]:
                ...
                fig_t.update_layout(title=f"Weekly Total {m}", height=300)
    ...
    with tabs[4]:
        ...
        with st.container():
            ...
            with c_ts1: 
                ...
                selected_games = st.multiselect("Select Weekend Matches", game_list_t, default=game_list_t[-3:] if len(game_list_t) >=3 else game_list_t, key="tourney_multi")
            with c_ts2: 
        ...
        if selected_games:
                ...
                for j in range(2):
                    if i + j < len(athletes_t):
                        ...
                        with card_cols[j]:
                            ...
                            fig_ath.update_layout(barmode='group', height=320, legend=dict(orientation="h", y=-0.2), template="simple_white")
            ...
            t_m_list = ['Total Jumps', 'Player Load', 'Estimated Distance', 'Explosive Efforts']
            team_avg_t = df[df['Session_Name'].isin(selected_games)].groupby(['Session_Name', 'Sheet_Order'])[t_m_list].mean().reset_index().sort_values('Sheet_Order')
            ...
            for idx, m in enumerate(t_m_list):
                with locs[idx]:
                    ...
                    fig_t.update_layout(title=f"Team Avg: {m}", height=400, template="simple_white", bargap=0.0)
                    st.plotly_chart(fig_t, use_container_width=True, config=LOCKED_CONFIG)
