...
try:
    ...
    st.markdown('<h1 style="color:#FF8200; text-align:center;">Lady Vol Volleyball Performance</h1>', unsafe_allow_html=True)
            ...
            with jc1:
                ...
                if not p_cmj_hist.empty:
                    ...
                    if latest['Test Date'] >= p_cmj_hist['Test Date'].max() - timedelta(days=10):
                        ...
                        label, color, profile = ("ELITE", "#28a745", "High Jumps and RSI") if cur_h >= base_h and cur_rsi >= base_rsi else ("FATIGUED", "#dc3545", "Low Jumps and RSI") if cur_h < base_h and cur_rsi < base_rsi else ("SPRINGY", "#ffc107", "Mixed Readiness")
                ...
                for j in range(2):
                    if i + j < len(ath_t):
                        ...
                        with card_cols[j]:
                            ...
                            for _, r in ad.iterrows(): card_html += f"<tr><td>{r['Session_Name']}</td><td>{int(r['Total Jumps'])}</td><td>{r['Player Load']:.0f}</td><td>{r['Estimated Distance (y)']:.0f}</td><td>{r['Explosive Efforts']:.0f}</td></tr>"
