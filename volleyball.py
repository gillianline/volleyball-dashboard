    ...
    try:
        ...
        with tabs[4]: # Match Summary
            ...
            if selected_matches:
                ...
                with tc1: 
                    fig_avg1 = px.bar(team_avg, x='Session_Name', y='Total Jumps', title="Avg Total Jumps", color='Session_Name', color_discrete_map=m_map)
                    st.plotly_chart(fig_avg1, use_container_width=True, config=LOCKED_CONFIG)
                with tc2: 
                    fig_avg2 = px.bar(team_avg, x='Session_Name', y='Player Load', title="Avg Player Load", color='Session_Name', color_discrete_map=m_map)
                    st.plotly_chart(fig_avg2, use_container_width=True, config=LOCKED_CONFIG)
