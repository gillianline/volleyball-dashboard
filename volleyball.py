...
try:
            ...
            with jc1:
                ...
                if not p_cmj_hist.empty:
                    ...
                    if latest['Test Date'] >= p_cmj_hist['Test Date'].max() - timedelta(days=10):
                        ...
                        label, color, profile = ("ELITE", "#28a745", "Jump Height and RSI are both High.") if cur_h >= base_h and cur_rsi >= base_rsi else ("FATIGUED", "#dc3545", "Jump Height and RSI are both Low.") if cur_h < base_h and cur_rsi < base_rsi else ("SPRINGY", "#ffc107", "Mixed Readiness Profile")
    ...
    # --- TAB 4: Match SUMMARY (ADDED PDF EXPORT) ---
    with tabs[4]:
        # PDF EXPORT BUTTON
        st.markdown('<div class="no-print">', unsafe_allow_html=True)
        if st.button("🖨️ Open PDF Print View"):
        ...
        if selected_matches:
            ...
            # Print optimization: fit more cards per row
                    ...
                    if i + j < len(athletes_t):
                        ...
                        with card_cols[j]:
                            ...
                            for _, r in ath_data_t.iterrows():
                                ...
                                fig_ath.add_trace(go.Bar(name=f"Distance", x=['Estimated Distance (y)'], y=[r['Estimated Distance (y)']], marker=dict(color=match_color_map[r['Session_Name']], opacity=0.6, line=dict(color=match_color_map[r['Session_Name']], width=2)), showlegend=False), secondary_y=True)
