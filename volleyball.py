...
if check_password():
    ...
    # ISOLATION REFACTOR: Unique local scopes generated right at the top level split
    df_active = raw_df[raw_df['Season'] == selected_season].copy()
    match_active = raw_match_df[raw_match_df['Season'] == selected_season].copy()
    cmj_active = cmj_df[cmj_df['Season'] == selected_season].copy()
    ash_active = ash_df[ash_df['Season'] == selected_season].copy()
    er_active = er_df[er_df['Season'] == selected_season].copy()
    phase_active = phase_df[phase_df['Season'] == selected_season].copy()
    ...
    master_athlete_list = sorted(list(set(df_active['Name'].unique()) | set(cmj_active['Name'].unique()) | set(ash_active['Name'].unique()) | set(er_active['Name'].unique())))
    session_list = df_active[df_active['Session_Name'].notna()].sort_values('Date', ascending=False)['Session_Name'].unique().tolist()
    ...
    with tabs[0]:
        ...
        with tab0:
            ...
            for s in session_list:
                s_date_series = df_active[df_active['Session_Name'] == s]['Date']
            ...
            if selected_session_prof == tournament_label:
                ...
                p_session_data = df_active[(df_active['Name'] == selected_athlete_prof) & (df_active['Date'] == curr_date_prof)].copy()
                ...
                p_session_data = df_active[(df_active['Name'] == selected_athlete_prof) & (df_active['Session_Name'] == selected_session_prof)]
            ...
            if p_row.empty:
                curr_date_prof = pd.to_datetime(target_date_str) if selected_session_prof == tournament_label else pd.to_datetime(df_active['Date'].max() if not df_active.empty else "2026-01-01")
                meta_lookup = df_active[df_active['Name'] == selected_athlete_prof]
            ...
            p_full_prof = df_active[df_active['Name'] == selected_athlete_prof]
            ...
            p_cmj_hist = cmj_active[(cmj_active['Name'] == selected_athlete_prof) & (cmj_active['Test Date'] <= curr_date_prof)].sort_values('Test Date')
            cmj_col, rsi_col = 'Jump Height (Imp-Mom) [cm]', 'RSI-modified [m/s]'

            with jc1:
                baseline_cmj = p_cmj_hist[p_cmj_hist['Season'] == 'Summer'].head(1) if selected_season == 'Summer' else cmj_active[(cmj_active['Name'] == selected_athlete_prof) & (cmj_active['Week'] == 4)]
            ...
            if not p_er_hist.empty:
                ...
                with ec1:
                    ...
                    if not baseline_er.empty:
                        ...
                        st.markdown(f'<div class="info-box" style="text-align:center; margin-top:10px;"><p style="margin:0;"><b>ROM Asymmetry:</b> {cur_asym_rom:+.1f}%</p></div>', unsafe_allow_html=True)
                with ec2:
                    fig_er = go.Figure()
    ...
    # ==========================================
    # --- ISOLATED TABS WORKSPACE LAYERS -------
    # ==========================================
    ...
    for idx, tab_name in enumerate(remaining_tabs, start=1):
        with tabs[idx]:
            ...
            with loop_container:
                st.subheader(f"{tab_name} Performance Matrix Layer")
                # THE OVERWRITE FIX: Isolate the table loops from the Tab 0 global environment entirely by naming its scoped dataframe uniquely
                if not df_active.empty:
                    # Enforce visualization rule: use Maximum position metrics instead of general season averages
                    summary_view_isolated = df_active.groupby('Position')[['Player Load', 'Total Jumps']].max().reset_index()
                    st.dataframe(summary_view_isolated, use_container_width=True, hide_index=True, key=f"tab_view_isolated_{idx}")
                else:
                    st.info("Metrics logging arrays initializing data parameters.")
