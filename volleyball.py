...
if check_password():
    ...
    try:
        ...
        with tabs[3]: # Tab 4: Game v Practice
            ...
            # --- 1. SELECTION FILTERS (Mode and Athlete/Position) ---
            ...
            # --- 2. DATA CLEANING (Ensuring Numeric Rates) ---
            def clean_gp_data(target_df):
                if target_df.empty: return target_df
                # Handle inconsistent naming across sheets
            ...
            if not main_filtered.empty and not match_filtered.empty:
                # Calculate Averages for the entire Season
                # Practice Data
                ...
                for m in calc_cols:
                    # Calculate rates (Volume / Minutes)
        ...
        with tabs[6]: # Tab 5: Work Index Matrix
            ...
            if phase_df is not None and not phase_df.empty:
                ...
                # --- THE DURATION SYNC FIX ---
                # We identify the 'Official Mins' for each drill on each date (the max time any player recorded)
                # This ensures the "Mins" column is identical for everyone in a specific drill
                working_matrix['Official_Mins'] = working_matrix.groupby(['Date', 'Phase'])[time_col].transform('max')

                # Calculate Rates based on the official drill clock
                ...
                # Aggregate using mean for rates, but use the official duration
                ...
                for _, row in matrix_df.iterrows():
                    m_mins = row['Official_Mins']
                    l_val = (row['Player Load_Rate'] * m_mins) if metric_mode == "Total Volume" else row['Player Load_Rate']
                    j_val = (row['Total Jumps_Rate'] * m_mins) if metric_mode == "Total Volume" else row['Total Jumps_Rate']
                    e_val = (row['Explosive Efforts_Rate'] * m_mins) if metric_mode == "Total Volume" else row['Explosive Efforts_Rate']

                    matrix_html += f"""<tr>
                                    <td style="padding: 10px; border: 1px solid #ddd;">{row[sort_col]}</td>
                                    <td style="padding: 10px; border: 1px solid #ddd;">{row['Phase']}</td>
                                    <td style="padding: 10px; border: 1px solid #ddd;">{m_mins:.1f}</td>
        ...
        with tabs[2]: # Tab 2: Performance History
            ...
            with sub_tabs[0]:
                ...
                # 2. COMBINATION STEP: Sum metrics and combine both Name and Type columns
                daily_raw = p_full.groupby(['Date', 'Week']).agg({
                    **{m: 'sum' for m in metrics_to_score},
                    'Session_Name': lambda x: ' | '.join(x.astype(str)),
                    'Session Type': lambda x: ' | '.join(x.astype(str)) # Added Session Type tracking
                ...
                for idx, row in daily_raw.iterrows():
                    ...
                    # THE FIX: Check both Session_Name AND Session Type for Match/Game keywords
                    # This ensures April 18th is caught even if the title is generic
                    name_str = str(row['Session_Name']).upper()
                    type_str = str(row['Session Type']).upper()
                ...
                # Layer 2: Match (Bold Orange - Now catches April 18th)
