...
st.markdown("""
    ...
    /* PDF PRINT LOGIC - Hides UI Elements and Specific Selection Headers */
    @media print {
        .stTabs [role="tablist"], 
        .no-print, 
...
try:
    ...
    # --- TAB 2: MATCH V PRACTICE (TREND LINE FIX) ---
    with tabs[2]:
        ...
        if not w_data.empty and not g_data_l.empty:
            ...
            st.markdown('<div class="section-header">Weekly Load Trend (Match-by-Match)</div>', unsafe_allow_html=True)
            wk_df = df[(df['Name'] == gp_p) & (df['Week'] == sel_w)].sort_values('Sheet_Order')
            # Fix: X is Session_Name so same-day matches appear as distinct points on one line
    ...
    with tabs[4]:
        ...
        if selected_matches:
            ...
            t_avgs = df[df['Session_Name'].isin(selected_matches)].groupby(['Session_Name', 'Sheet_Order'])[['Total Jumps', 'Player Load', 'Estimated Distance (y)', 'Explosive Efforts']].mean().reset_index().sort_values('Sheet_Order')
            
            # Locked 2x2 grid for team graphs
            c1, c2 = st.columns(2); c3, c4 = st.columns(2)
            cols = [c1, c2, c3, c4]
            for idx, m in enumerate(['Total Jumps', 'Player Load', 'Estimated Distance (y)', 'Explosive Efforts']):
                with cols[idx]:
                    fig_t = go.Figure()
                    fig_t.add_trace(go.Bar(x=t_avgs['Session_Name'], y=t_avgs[m], marker_color=[match_color_map[g] for g in t_avgs['Session_Name']], marker_line_width=0))
