                ...
                for j in range(2):
                    if i + j < len(athletes_t):
                        ...
                        with card_cols[j]:
                            ...
                            card_html += f"<tr style='background:#4895DB; color:white; font-weight:900;'><td>TOTAL</td><td>{int(ath_data_t['Total Jumps'].sum())}</td><td>{ath_data_t['Player Load'].sum():.0f}</td><td>{ath_data_t['Explosive Efforts'].sum():.0f}</td><td>{ath_data_t['Estimated Distance (y)'].sum():.0f}</td></tr></tbody></table></div>"
