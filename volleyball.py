...
try:
            ...
            with jc1:
                ...
                if not p_cmj_hist.empty:
                    ...
                    if latest['Test Date'] >= p_cmj_hist['Test Date'].max() - timedelta(days=10):
                        ...
                        label, color, profile = ("ELITE", "#28a745", "High Height & RSI"), ("FATIGUED", "#dc3545", "Low Height & RSI"), ("GRINDER", "#ffc107", "Mixed Readiness")
    ...
    with tabs[2]:
        ...
        if not w_data.empty and not g_data_l.empty:
            ...
            st.markdown('<div class="section-header">Weekly Load Trend Leading to Match</div>', unsafe_allow_html=True); wk_df = df[(df['Name'] == gp_p) & (df['Week'] == sel_w)].sort_values('Date'); fig_trend = px.line(wk_df, x='Date', y='Player Load', markers=True); fig_trend.update_traces(line_color='#FF8200', marker_size=12); st.plotly_chart(fig_trend, use_container_width=True, config=LOCKED_CONFIG)
    ...
    # --- TAB 4: Match SUMMARY (PDF EXPORT + MULTI-MATCH FIX) ---
    with tabs[4]:
        st.sidebar.markdown("---")
        if st.sidebar.button("🖨️ Prepare PDF for Printing"):
        ...
        if selected_matches:
            ...
            for i in range(0, len(athletes_t), 2):
                ...
                for j in range(2):
                    if i + j < len(athletes_t):
                        ...
                        with card_cols[j]:
                            card_html = f'<div class="gallery-card"><div style="display:flex; align-items:center; gap:15px; padding:15px; background:#f8f9fa; border-bottom:2px solid #FF8200;"><img src="{ath_data_t["PhotoURL"].iloc[0]}" style="width:70px; border-radius:50%;"><div><p style="margin:0; font-weight:900; font-size:18px;">{ath_name_t}</p><p style="margin:0; color:#4895DB; font-weight:700;">{ath_data_t["Position"].iloc[0]}</p></div></div><div style="padding:10px 15px;"><table class="scout-table"><thead><tr><th>Match</th><th>Total Jumps</th><th>Player Load</th><th>Estimated Distance</th><th>Explosive Efforts</th></tr></thead><tbody>'
                            for _, r in ath_data_t.iterrows(): card_html += f"<tr><td style='font-weight:700;'>{r['Session_Name']}</td><td>{int(r['Total Jumps'])}</td><td>{r['Player Load']:.0f}</td><td>{r['Estimated Distance (y)']:.0f}</td><td>{r['Explosive Efforts']:.0f}</td></tr>"
                            card_html += f"<tr style='background:#4895DB; color:white; font-weight:900;'><td>TOTAL</td><td>{int(ath_data_t['Total Jumps'].sum())}</td><td>{ath_data_t['Player Load'].sum():.0f}</td><td>{ath_data_t['Estimated Distance (y)'].sum():.0f}</td><td>{ath_data_t['Explosive Efforts'].sum():.0f}</td></tr></tbody></table></div>"
            ...
            st.write("<br>", unsafe_allow_html=True); st.markdown('<div class="section-header">Team Match Averages (Excel Order)</div>', unsafe_allow_html=True)
            t_m_list = ['Total Jumps', 'Player Load', 'Estimated Distance (y)', 'Explosive Efforts']; team_avg_t = df[df['Session_Name'].isin(selected_matches)].groupby(['Session_Name', 'Sheet_Order'])[t_m_list].mean().reset_index().sort_values('Sheet_Order')
            ...
            for idx, m in enumerate(t_m_list):
                with [r1c1, r1c2, r2c1, r2c2][idx]:
                    ...
                    fig_t.update_layout(title=f"Team Avg: {m}", showlegend=False, height=400, margin=dict(l=60, r=40, t=60, b=100), template="simple_white", bargap=0.0); st.plotly_chart(fig_t, use_container_width=True, config=LOCKED_CONFIG)
