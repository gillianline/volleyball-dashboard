...
st.markdown("""
    ...
    /* PDF PRINT LOGIC: Hides UI and Specific Selection Headers */
    @media print {
        .stTabs [role="tablist"], 
        .no-print, 
...
try:
    ...
    # --- TAB 0, 1, 2, 3: RESTORED ORIGINAL CONTENT ---
    with tabs[0]:
    ...
    with tabs[2]:
        ...
        if not w_data.empty and not g_data_l.empty:
            ...
            with cg2:
                fig_v = make_subplots(specs=[[{"secondary_y": True}]]); fig_v.add_trace(go.Bar(x=low_m, y=[w_avg[m] for m in low_m], name="Weekly Avg", marker_color='#4895DB'), secondary_y=False); fig_v.add_trace(go.Bar(x=low_m, y=[g_d[m] for m in low_m], name=f"Match Output", marker_color='#FF8200'), secondary_y=False); st.plotly_chart(fig_v, use_container_width=True)
    ...
    with tabs[4]:
        ...
        if st.button("🖨️ Prepare for PDF Printing"):
            st.markdown('<script>window.print();</script>', unsafe_allow_html=True)
        
        # Hide this header and the widgets specifically when printing
        st.markdown('<div class="print-hide"><div class="section-header">Match Comparison Selection</div>', unsafe_allow_html=True)
        ...
        if selected_matches:
                    ...
                    if i + j < len(athletes_t):
                        ...
                        with card_cols[j]:
                            ...
                            <div class="gallery-card">
                                ...
                                <div style="padding:10px 15px;"><table class="scout-table"><thead><tr><th>Match</th><th>Total Jumps</th><th>Player Load</th><th>Explosive Efforts</th></tr></thead><tbody>
                            """
                            ...
                            card_html += f"<tr style='background:#4895DB; color:white; font-weight:900;'><td>TOTAL</td><td>{int(ath_data_t['Total Jumps'].sum())}</td><td>{ath_data_t['Player Load'].sum():.0f}</td><td>{ath_data_t['Explosive Efforts'].sum():.0f}</td></tr></tbody></table></div>"
            ...
            r1, r2 = st.columns(2); r3, r4 = st.columns(2); locs = [r1, r2, r3, r4]
