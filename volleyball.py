...
# --- CSS: FORMATTING & HIGHLIGHTING (RESTORED EXACTLY) ---
st.markdown("""
    <style>
    ...
    @media print {
        .stTabs [role="tablist"], .no-print, [data-testid="stSidebar"], header { display: none !important; }
...
try:
            ...
            with jc1:
                ...
                if not p_cmj_hist.empty:
                    ...
                    if latest['Test Date'] >= p_cmj_hist['Test Date'].max() - timedelta(days=10):
                        ...
                        label, color, profile = ("ELITE", "#28a745", "High Readiness"), ("FATIGUED", "#dc3545", "Low Readiness"), ("GRINDER", "#ffc107", "Mixed Readiness")
                        st.markdown(f'<div style="text-align:center;"><div class="score-box" style="background-color:{color if p_diff > 0 else "#dc3545"};">{p_diff:+.1f}%<span style="font-size:10px; display:block;">{label}</span></div></div><div class="info-box"><b>Latest Test ({latest["Test Date"].strftime("%m/%d")}):</b> {cur_h:.1f}" | {cur_rsi:.2f} RSI<br><b>Profile:</b> {profile}</div>', unsafe_allow_html=True)
            with jc2:
                if not p_cmj_hist.empty:
    ...
    with tabs[2]:
        ...
        with c_gg: 
            match_opts = df[(df['Name'] == gp_p) & (df['Session_Type'] == 'Match') & (df['Week'] == sel_w)]['Session_Name'].unique(); gp_g = st.selectbox("Select Match", match_opts, key="gp_g_vf")
        ...
        if not w_data.empty and not g_data_l.empty:
            ...
            with cg2:
                fig_v = make_subplots(specs=[[{"secondary_y": True}]]); fig_v.add_trace(go.Bar(x=low_m, y=[w_avg[m] for m in low_m], name="Practice", marker_color='#4895DB'), secondary_y=False); fig_v.add_trace(go.Bar(x=low_m, y=[g_d[m] for m in low_m], name="Match", marker_color='#FF8200'), secondary_y=False); fig_v.add_trace(go.Bar(x=['Estimated Distance (y)'], y=[w_avg['Estimated Distance (y)']], name="Dist-Pract", marker=dict(color='#4895DB', opacity=0.6, line=dict(color='#4895DB', width=1))), secondary_y=True); fig_v.add_trace(go.Bar(x=['Estimated Distance (y)'], y=[g_d['Estimated Distance (y)']], name="Dist-Match", marker=dict(color='#FF8200', opacity=0.6, line=dict(color='#FF8200', width=1))), secondary_y=True); fig_v.update_layout(height=400, barmode='group'); st.plotly_chart(fig_v, use_container_width=True)
            
            st.markdown('<div class="section-header">Weekly Load Trend (Connected Matches)</div>', unsafe_allow_html=True)
            wk_df = df[(df['Name'] == gp_p) & (df['Week'] == sel_w)].sort_values('Sheet_Order')
            fig_trend = go.Figure(); fig_trend.add_trace(go.Scatter(x=wk_df['Session_Name'], y=wk_df['Player Load'], mode='lines+markers', line=dict(color='#FF8200', width=3), marker=dict(size=12))); fig_trend.update_layout(height=400, template="simple_white"); st.plotly_chart(fig_trend, use_container_width=True, config=LOCKED_CONFIG)
    ...
    with tabs[3]:
        ...
        sel_p_pos = st.selectbox("Select Athlete for Comparative Trend", sorted(df['Name'].unique())); p_pos = df[df['Name'] == sel_p_pos].iloc[0]; pos_label = p_pos['Position']; max_wk = df['Week'].max(); rec_4 = list(range(int(max_wk)-3, int(max_wk)+1)); tr_df = df[df['Week'].isin(rec_4)]; t_col1, t_col2, t_col3 = st.columns(3)
        for i, m in enumerate(["Player Load", "Estimated Distance (y)", "Total Jumps"]):
            with [t_col1, t_col2, t_col3][i]:
                fig_t = go.Figure(); p_t = tr_df[tr_df['Name'] == sel_p_pos].groupby('Week')[m].sum().reset_index(); fig_t.add_trace(go.Scatter(x=p_t['Week'], y=p_t[m], name=sel_p_pos, line=dict(color='#0046ad', width=4))); pos_t = tr_df[tr_df['Position'] == pos_label].groupby(['Week', 'Name'])[m].sum().reset_index().groupby('Week')[m].mean().reset_index(); fig_t.add_trace(go.Scatter(x=pos_t['Week'], y=pos_t[m], name="Pos Avg", line=dict(color='#ff7f0e', dash='dash'))); fig_t.update_layout(title=f"Weekly Total {m}", height=300); st.plotly_chart(fig_t, use_container_width=True, config=LOCKED_CONFIG)
    ...
    with tabs[4]:
        ...
        with c_ts1:
            ...
            selected_matches = st.multiselect("Select Matches", match_list_t, default=match_list_t[-3:] if len(match_list_t) >=3 else match_list_t)
        if selected_matches:
            ...
            st.markdown('<div class="section-header">Athlete Match Breakdown</div>', unsafe_allow_html=True)
            t_df = df[df['Session_Name'].isin(selected_matches)].sort_values('Sheet_Order'); ath_list = sorted(t_df['Name'].unique())
            for i in range(0, len(ath_list), 2):
                card_cols = st.columns(2)
                for j in range(2):
                    if i + j < len(ath_list):
                        name = ath_list[i+j]; ad = t_df[t_df['Name'] == name]
                        with card_cols[j]:
                            card_html = f'<div class="gallery-card"><div style="display:flex; align-items:center; gap:15px; padding:15px; background:#f8f9fa; border-bottom:2px solid #FF8200;"><img src="{ad["PhotoURL"].iloc[0]}" style="width:70px; border-radius:50%;"><div><p style="margin:0; font-weight:900; font-size:18px;">{name}</p><p style="margin:0; color:#4895DB; font-weight:700;">{ad["Position"].iloc[0]}</p></div></div><div style="padding:10px 15px;"><table class="scout-table"><thead><tr><th>Match</th><th>Total Jumps</th><th>Player Load</th><th>Explosive Efforts</th></tr></thead><tbody>'
                            for _, r in ad.iterrows(): card_html += f"<tr><td>{r['Session_Name']}</td><td>{int(r['Total Jumps'])}</td><td>{r['Player Load']:.0f}</td><td>{r['Explosive Efforts']:.0f}</td></tr>"
                            ...
                            for _, r in ad.iterrows():
                                ...
                                fig_ath.add_trace(go.Bar(name=f"Dist", x=['Estimated Distance'], y=[r['Estimated Distance (y)']], marker=dict(color=match_color_map[r['Session_Name']], opacity=0.6, line=dict(color=match_color_map[r['Session_Name']], width=2)), showlegend=False), secondary_y=True)
            ...
            avg_df = t_df.groupby(['Session_Name', 'Sheet_Order'])[['Total Jumps', 'Player Load', 'Estimated Distance (y)', 'Explosive Efforts']].mean().reset_index().sort_values('Sheet_Order')
            ...
            for idx, m in enumerate(['Total Jumps', 'Player Load', 'Estimated Distance (y)', 'Explosive Efforts']):
                with [r1, r2, r3, r4][idx]:
                    fig = go.Figure(); fig.add_trace(go.Bar(x=avg_df['Session_Name'], y=avg_df[m], marker_color=[match_color_map[g] for g in avg_df['Session_Name']], marker_line_width=0))
                    fig.update_layout(title=f"Team Avg: {m}", template="simple_white", bargap=0.0); st.plotly_chart(fig, use_container_width=True, config=LOCKED_CONFIG)
