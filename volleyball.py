...
if check_password():
    ...
    # Base Data Isolation
    df_base = raw_df[raw_df['Season'] == selected_season].copy()
    match_base = raw_match_df[raw_match_df['Season'] == selected_season].copy()
    cmj_base = cmj_df[cmj_df['Season'] == selected_season].copy()
    ash_base = ash_df[ash_df['Season'] == selected_season].copy()
    er_base = er_df[er_df['Season'] == selected_season].copy()
    phase_base = phase_df[phase_df['Season'] == selected_season].copy()
    ...
    # Global Tracking Navigation via Session State to kill cross-talk
    tabs_list = ["Individual Profile", "Practice Scores", "Daily Combined Scores", "Spring Max vs Daily Combined", "Practice History", "Match v. Practice", "Match Summary", "Position Analysis", "Phase Analysis", "Practice Planner", "Spring v. Summer"]
    
    if "current_tab" not in st.session_state:
        st.session_state["current_tab"] = tabs_list[0]

    # Render a native radio header acting as an isolated tab selector navigation bar
    chosen_tab = st.radio("Navigation Control Panel", tabs_list, index=tabs_list.index(st.session_state["current_tab"]), horizontal=True, key="master_tab_navigator")
    st.session_state["current_tab"] = chosen_tab
    
    master_athlete_list = sorted(list(set(df_base['Name'].unique()) | set(cmj_base['Name'].unique()) | set(ash_base['Name'].unique()) | set(er_base['Name'].unique())))
    session_list = df_base[df_base['Session_Name'].notna()].sort_values('Date', ascending=False)['Session_Name'].unique().tolist()

    # ==========================================================
    # --- ISOLATION LOGIC GATES: ZERO CODE CROSS-TALK POSSIBLE ---
    # ==========================================================
    
    if chosen_tab == "Individual Profile":
        st.markdown("## Individual Profile View")
        ...
        for s in session_list:
            s_date_series = df_base[df_base['Session_Name'] == s]['Date']
        ...
        with c_prof1:
            selected_session_prof = c_prof1.selectbox("Session Selection", clean_session_list_prof, index=0, key="nav_sel_prof_isolated")
        with c_prof2:
            selected_athlete_prof = c_prof2.selectbox("Athlete Selection", master_athlete_list, key="nav_ath_prof_isolated")

        if selected_session_prof == tournament_label:
            curr_date_prof = pd.to_datetime(target_date_str)
            p_session_data = df_base[(df_base['Name'] == selected_athlete_prof) & (df_base['Date'] == curr_date_prof)].copy()
            ...
            p_session_data = df_base[(df_base['Name'] == selected_athlete_prof) & (df_base['Session_Name'] == selected_session_prof)]
        ...
        if p_row.empty:
            curr_date_prof = pd.to_datetime(target_date_str) if selected_session_prof == tournament_label else pd.to_datetime(df_base['Date'].max() if not df_base.empty else "2026-01-01")
            meta_lookup = df_base[df_base['Name'] == selected_athlete_prof]
        ...
        p_full_prof = df_base[df_base['Name'] == selected_athlete_prof]
        ...
        p_cmj_hist = cmj_base[(cmj_base['Name'] == selected_athlete_prof) & (cmj_base['Test Date'] <= curr_date_prof)].sort_values('Test Date')
        cmj_col = 'Jump Height (Imp-Mom) [cm]'
        rsi_col = 'RSI-modified [m/s]'

        with jc1:
            baseline_cmj = p_cmj_hist[p_cmj_hist['Season'] == 'Summer'].head(1) if selected_season == 'Summer' else cmj_base[(cmj_base['Name'] == selected_athlete_prof) & (cmj_base['Week'] == 4)]
        ...
        with jc2:
            if not p_cmj_hist.empty:
                ...
                st.plotly_chart(fig, use_container_width=True, config=LOCKED_CONFIG, key="cmj_top_chart_scoped")
        ...
        p_ash_all = ash_active[(ash_active['Name'] == selected_athlete_prof) & (ash_active['Test Date'] <= curr_date_prof)].sort_values('Test Date')
        ...
        if not p_ash_all.empty:
            ...
            with ac1:
                ...
                with asc1:
                    asc1.markdown(f'<div class="score-box" style="background-color:{color_ash_l}; line-height:1.2; padding-top:15px; height:80px; width:100%;"><span style="font-size:18px;">{li:.0f} N</span><span style="font-size:10px; display:block; font-weight:bold; margin-top:2px;">LEFT</span></div>', unsafe_allow_html=True)
                with asc2:
                    asc2.markdown(f'<div class="score-box" style="background-color:{color_ash_r}; line-height:1.2; padding-top:15px; height:80px; width:100%;"><span style="font-size:18px;">{ri:.0f} N</span><span style="font-size:10px; display:block; font-weight:bold; margin-top:2px;">RIGHT</span></div>', unsafe_allow_html=True)

                st.markdown(f'<div class="info-box" style="text-align:center; margin-top:10px;"><p style="margin:0; font-size:11px; color:grey;"><b>Asymmetry:</b> {asym_i:+.1f}% | <b>Base Shift:</b> L: {pct_l:+.1f}% / R: {pct_r:+.1f}%</p></div>', unsafe_allow_html=True)
            with ac2:
                ...
                if not p_ash_i_only.empty:
                    ...
                    st.plotly_chart(fig_ash, use_container_width=True, config=LOCKED_CONFIG, key="ash_profile_chart_scoped")
        else:
        ...
        p_er_hist = er_active[(er_active['Name'] == selected_athlete_prof) & (er_active['Test Date'] <= curr_date_prof)].sort_values('Test Date')
        ...
        if not p_er_hist.empty:
            ...
            with ec1:
                ...
                if not baseline_er.empty:
                    ...
                    st.markdown(f'<div class="info-box" style="text-align:center; margin-top:10px;"><p style="margin:0;"><b>ROM Asymmetry Index:</b> {cur_asym_rom:+.1f}%</p></div>', unsafe_allow_html=True)
            with ec2:
                fig_er = go.Figure()
    ...
    # ======================================================================
    # --- HARD ISOLATED WORKING TABS: BLOCKING RE-RUN OVERWRITES ------------
    # ======================================================================
    elif chosen_tab == "Practice Scores":
        st.subheader("Practice Scores Overview")
        if not df_active.empty:
            summary_prac = df_active[df_active['Session_Type'] == 'Practice'].groupby('Position')[['Player Load', 'Total Jumps']].max().reset_index()
            st.dataframe(summary_view_isolated, use_container_width=True, hide_index=True)
            
    elif chosen_tab == "Daily Combined Scores":
        st.subheader("Daily Combined Analytics")
        if not df_active.empty:
            daily_comb = df_active.groupby('Date')[['Player Load', 'Total Jumps']].max().reset_index()
            st.dataframe(daily_comb, use_container_width=True, hide_index=True)

    elif chosen_tab == "Spring Max vs Daily Combined":
        st.subheader("Spring Threshold Comparison Performance Profiles")
        if not df_active.empty:
            max_comp_view = df_active.groupby('Position')[['Player Load', 'Total Jumps']].max().reset_index()
            st.dataframe(max_comp_view, use_container_width=True, hide_index=True)

    elif chosen_tab == "Practice History":
        st.subheader("Practice History Logging")
        if not df_active
