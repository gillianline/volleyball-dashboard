...
if check_password():
    ...
    st.markdown("""
        ...
        /* PREVENT SPLITTING ATHLETES ON PRINT */
        ...
        @media print {
            ...
            /* Surgical removal of UI items only */
            .stTabs [role="tablist"], [data-testid="stSidebar"], header, footer, button, .print-hide { 
    ...
    try:
        ...
        with c_nav1: selected_session = st.selectbox("Practice Selection", session_list, index=0, key="nav_sel_ind")
        with c_nav2: pos_f = st.selectbox("Position Filter", ["All Positions"] + sorted([p for p in df['Position'].unique() if p != "N/A"]), key="nav_pos_ind")

        with tabs[0]: # Tab 0 Restored
            ...
            if not day_df.empty:
                ...
                with jc1:
                    p_cmj_hist = cmj_df[(cmj_df['Athlete'] == sel_p) & (cmj_df['Test Date'] <= p['Date'])].sort_values('Test Date')
                    sync_cmj = p_cmj_hist[(p_cmj_hist['Test Date'] > p['Date'] - timedelta(days=7))]
        ...
        with tabs[4]: # Tab 4 (Print Fix only)
