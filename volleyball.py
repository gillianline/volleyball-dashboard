...
if check_password():
            ...
            if not p_er_hist.empty:
                ...
                with ec1:
                    ...
                    if not baseline_er.empty:
                        ...
                        st.markdown(f'<div class="info-box" style="text-align:center; margin-top:10px;"><p style="margin:0; font-size:11px; color:grey;"><b>ROM Asymmetry:</b> {cur_asym_rom:+.1f}%</p></div>', unsafe_allow_html=True)
                with ec2:
                    fig_er = go.Figure()
    ...
    # ==========================================
    # --- TABS 1 TO 10: STRICT KEY ISOLATION ---
    # ==========================================
    remaining_tabs = [
    ...
    for idx, tab_name in enumerate(remaining_tabs, start=1):
        with tabs[idx]:
            # Each individual workspace segment gets an isolated local layout frame container
            loop_container = st.container()
            with loop_container:
                st.subheader(f"{tab_name} Analysis Dashboard Layer")
                if not df.empty:
                    # Enforce the operational visual rule: use maximum benchmarks, not season averages
                    summary_view = df.groupby('Position')[['Player Load', 'Total Jumps']].max().reset_index()
                    st.dataframe(summary_view, use_container_width=True, hide_index=True, key=f"isolated_df_key_{idx}")
                else:
                    st.info("Tracking log files currently initializing parameters.")
