...
try:
    ...
    # Session lists sorted by original sheet order
    ...
    with tabs[4]:
        ...
        if selected_games:
                ...
                for j in range(2):
                    if i + j < len(athletes_t):
                        ...
                        with card_cols[j]:
                            ...
                            card_html += f"<tr style='background:#4895DB; color:white; font-weight:900;'><td>TOTAL</td><td>{int(ath_data_t['Total Jumps'].sum())}</td><td>{ath_data_t['Player Load'].sum():.0f}</td><td>{ath_data_t['Estimated Distance'].sum():.0f}</td><td>{ath_data_t['Explosive Efforts'].sum():.0f}</td></tr></tbody></table>"
                            ...
                            for _, r in ath_data_t.iterrows():
                                ...
                                fig_ath.add_trace(go.Scatter(name=f"{r['Session_Name']} Dist", x=['Estimated Distance'], y=[r['Estimated Distance']], mode='markers+text', marker=dict(size=12, color=game_color_map[r['Session_Name']], symbol='diamond'), text=[f"{int(r['Estimated Distance'])}"], textposition="top center"), secondary_y=True)
                            ...
                            fig_ath.update_yaxes(title_text="Distance (Right Axis)", secondary_y=True)
            ...
            for idx, m in enumerate(t_m_list):
                with locs[idx]:
                    fig_t = go.Figure()
                    fig_t.add_trace(go.Bar(x=team_avg_t['Session_Name'], y=team_avg_t[m], marker_color=[game_color_map[g] for g in team_avg_t['Session_Name']]))
                    fig_t.update_layout(title=f"Team Avg: {m}", showlegend=False, height=400, margin=dict(l=60, r=40, t=60, b=100), template="simple_white", bargap=0.01)
                    st.plotly_chart(fig_t, use_container_width=True, config=LOCKED_CONFIG)
