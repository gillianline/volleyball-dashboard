...
try:
    ...
    with tabs[2]:
        ...
        if not w_data.empty and not g_data_l.empty:
            ...
            fig_trend = px.line(wk_df, x='Session_Name', y='Player Load', markers=True)
    ...
    with tabs[4]:
        ...
        if st.button("🖨️ Prepare Report"): st.markdown('<script>window.print();</script>', unsafe_allow_html=True)
            ...
            for i in range(0, len(ath_t), 2):
                ...
                for j in range(2):
                    if i + j < len(ath_t):
                        ...
                        with card_cols[j]:
                            card_html = f"""<div class="gallery-card"><div style="display:flex; align-items:center; gap:15px; padding:15px; background:#f8f9fa; border-bottom:2px solid #FF8200;"><img src="{ad['PhotoURL'].iloc[0]}" class="gallery-photo"><div><p style="margin:0; font-weight:900; color:#1D1D1F; font-size:18px;">{name}</p><p style="margin:0; color:#4895DB; font-weight:700; font-size:12px;">{ad['Position'].iloc[0]}</p></div></div><div style="padding:10px 15px;"><table class="scout-table" style="margin-bottom:0;"><thead><tr><th>Match</th><th>Total Jumps</th><th>Player Load</th><th>Est Dist (y)</th><th>Explosive Efforts</th></tr></thead><tbody>"""
                            for _, r in ad.iterrows(): card_html += f"<tr><td style='font-weight:700;'>{r['Session_Name']}</td><td>{int(r['Total Jumps'])}</td><td>{r['Player Load']:.0f}</td><td>{r['Estimated Distance (y)']:.0f}</td><td>{r['Explosive Efforts']:.0f}</td></tr>"
