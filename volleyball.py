...
try:
    ...
    # --- TAB 0 & 1 (With Top Filters) ---
    with tabs[0]:
        session_map = df[['Date', 'Session_Name']].drop_duplicates().sort_values('Date', ascending=False)
        col_f1, col_f2 = st.columns(2)
        with col_f1: selected_session = st.selectbox("Practice Selection", session_map['Session_Name'].tolist(), index=0, key="p_sel_0")
        with col_f2: pos_f = st.selectbox("Position Filter", ["All Positions"] + sorted([p for p in df['Position'].unique() if p != "N/A"]), key="pos_0")
    ...
    with tabs[1]:
        # Using session variables from first tab logic for consistency
        if not day_df.empty:
    ...
    # --- TAB 2 (Clean Top, Tab-Specific Filters Only) ---
    with tabs[2]:
