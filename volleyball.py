...
try:
    ...
    # --- PRINT MODE TOGGLE ---
    st.sidebar.markdown("### 🖨️ PDF Settings")
    print_mode = st.sidebar.checkbox("Ready to Print?", help="Check this to hide the UI for a clean PDF.")

    if not print_mode:
        tabs = st.tabs(["Individual Profile", "Team Gallery", "Match v. Practice", "Position Analysis", "Match Summary"])
    else:
        st.info("💡 Print Mode Active. Press Cmd+P or Ctrl+P to save as PDF. Content limited to Match Summary.")
    ...
    if not print_mode:
        ...
        with tabs[2]:
            ...
            if not w_data.empty and not g_data_l.empty:
                ...
                with cg2:
                    fig_v = make_subplots(specs=[[{"secondary_y": True}]]); fig_v.add_trace(go.Bar(x=low_m, y=[w_avg[m] for m in low_m], name="Practice", marker_color='#4895DB'), secondary_y=False); fig_v.add_trace(go.Bar(x=low_m, y=[g_d[m] for m in low_m], name="Match", marker_color='#FF8200'), secondary_y=False); st.plotly_chart(fig_v, use_container_width=True)
    ...
    # --- TAB 4: MATCH SUMMARY CONTENT ---
    summary_container = tabs[4] if not print_mode else st.container()

    with summary_container:
        # Only show controls if NOT printing
        if not print_mode:
            ...
            with c_ts1:
                ...
                selected_matches = st.multiselect("Select Weekend Matches", match_list_t, default=match_list_t[-3:] if len(match_list_t) >=3 else match_list_t, key="match_multi")
            with c_ts2:
                pos_filter_t = st.selectbox("Filter by Position", ["All Positions"] + sorted(list(df['Position'].unique())), key="match_pos_filter")
        else:
            selected_matches = st.session_state.get("match_multi", [])
            pos_filter_t = st.session_state.get("match_pos_filter", "All Positions")
        ...
        if selected_matches:
            ...
            # Layout logic: 3 columns if printing to save space, else 2
                        ...
                        with card_cols[j]:
                            ...
                            <div class="gallery-card">
                                <div style="display:flex; align-items:center; gap:10px; padding:10px; background:#f8f9fa; border-bottom:2px solid #FF8200;">
                                    <img src="{ath_data_t['PhotoURL'].iloc[0]}" class="gallery-photo" style="width:70px; height:70px;">
                                    <div>
                                ...
                                <div style="padding:5px;">
                                    <table class="scout-table" style="font-size:9px;">
                                        <thead><tr><th>Match</th><th>Jumps</th><th>Load</th><th>Dist</th><th>Effort</th></tr></thead>
                                        <tbody>
                            """
                            for _, r in ath_data_t.iterrows():
                                card_html += f"<tr><td>{r['Session_Name'][:10]}</td><td>{int(r['Total Jumps'])}</td><td>{r['Player Load']:.0f}</td><td>{r['Estimated Distance (y)']:.0f}</td><td>{r['Explosive Efforts']:.0f}</td></tr>"
                            ...
                            fig_ath.update_layout(barmode='group', height=200, margin=dict(l=10,r=10,t=10,b=10), showlegend=False, template="simple_white")
                            st.plotly_chart(fig_ath, use_container_width=True, config=LOCKED_CONFIG)
                            st.markdown("</div>", unsafe_allow_html=True)

            st.write("<br>", unsafe_allow_html=True)
            st.markdown('<div class="section-header">Team Match Averages (Excel Order)</div>', unsafe_allow_html=True)
            
            # --- FIXED SAME DAY COMPARISON LOGIC ---
            # Grouping by Session_Name but including Sheet_Order so multiple matches on same date are distinct bars
            ...
            metrics_t = ['Total Jumps', 'Player Load', 'Estimated Distance (y)', 'Explosive Efforts']
            
            for idx, m in enumerate(metrics_t):
                with locs[idx]:
                    fig_t = go.Figure()
                    fig_t.add_trace(go.Bar(
                        x=team_avg_t['Session_Name'], 
                        y=team_avg_t[m], 
                        marker_color=[match_color_map[g] for g in team_avg_t['Session_Name']], 
                        marker_line_width=0
                    ))
                    fig_t.update_layout(title=f"Team Avg: {m}", showlegend=False, height=350, margin=dict(l=40, r=20, t=40, b=80), template="simple_white", bargap=0.0)
