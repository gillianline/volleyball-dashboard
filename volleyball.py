...
try:
    ...
    with tab_ind:
        ...
        if not day_df.empty:
            ...
            # Filter CMJ history up to SELECTED practice date
            ...
            with c3:
                st.markdown(f'<div class="score-wrapper"><div class="score-label">Practice Score</div><div class="score-box" style="background-color:{get_gradient(practice_score)};">{practice_score}</div></div>', unsafe_allow_html=True)
            ...
            with jc1:
                if not sync_cmj.empty:
                    ...
                    if cur_h >= base_h and cur_rsi >= base_rsi: label, color, why = "ELITE", "#28a745", "Jump Height and RSI are both High."
                    elif cur_h >= base_h and cur_rsi < base_rsi: label, color, why = "GRINDER", "#ffc107", "Jump Height is High | RSI is Low."
                    elif cur_h < base_h and cur_rsi >= base_rsi: label, color, why = "SPRINGY", "#ffc107", "Jump Height is Low | RSI is High."
                    else: label, color, why = "FATIGUED", "#dc3545", "Jump Height and RSI are both Low."
                    
                    prev_h, prev_rsi = "N/A", "N/A"
                    if len(p_cmj_hist) > 1:
                        prev = p_cmj_hist.iloc[-2]; prev_h, prev_rsi = f"{prev['Jump Height (in)']:.1f}\"", f"{prev['RSI-modified [m/s]']:.2f}"

                    st.markdown(f'<div class="score-wrapper"><div class="score-label">Readiness</div><div class="score-box" style="background-color:{color};">{p_diff:+.1f}%<span class="status-subtext">{label}</span></div></div><div class="info-box"><b>Today:</b> {cur_h:.1f}" | {cur_rsi:.2f} RSI<br><b>Previous:</b> {prev_h} | {prev_rsi} RSI<br><b>Profile:</b> {why}</div>', unsafe_allow_html=True)
            with jc2:
    ...
    with tab_gal:
        # Uses the session selected in the first tab for consistency
    ...
    # --- GAME V PRACTICE TAB (INDEPENDENT) ---
    with tab_gp:
        ...
        with c_w:
            ...
            gp_w = st.selectbox("Week", w_r['L'].tolist(), key="gp_w")
        ...
        g_data = df[(df['Name'] == gp_p) & (df['Session_Name'] == gp_g)].iloc[0]
        
        if not w_data.empty:
            w_avg = w_data[crit].mean(); cg1, cg2 = st.columns([1, 2])
            with cg1:
                for m in crit:
