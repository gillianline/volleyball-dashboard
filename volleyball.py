...
st.markdown("""
    ...
    @media print {
        ...
        /* Hide all selection widgets, labels, and the comparison selection section */
        .print-hide,
...
try:
    ...
    # --- TAB 0: INDIVIDUAL PROFILE (UNTOUCHED) ---
    with tabs[0]:
        ...
        if not day_df.empty:
            ...
            if not p_phases.empty:
                ...
                for _, r in p_phases.iterrows(): p_tbl += f"<tr><td>{r['Phase']}</td><td>{int(r['Total Jumps'])}</td><td>{r['Player Load']:.1f}</td></tr>"
    ...
    with tabs[2]:
        ...
        if not w_data.empty and not g_data_l.empty:
            ...
            with cg2:
                fig_dual = make_subplots(specs=[[{"secondary_y": True}]]); fig_dual.add_trace(go.Bar(x=low_m, y=[w_avg[m] for m in low_m], name="Weekly Avg", marker_color='#4895DB'), secondary_y=False); fig_dual.add_trace(go.Bar(x=low_m, y=[g_d[m] for m in low_m], name=f"Game Output", marker_color='#FF8200'), secondary_y=False); fig_dual.update_layout(height=400, barmode='group'); st.plotly_chart(fig_dual, use_container_width=True, config=LOCKED_CONFIG)
            wk_trends = df[df['Week'] == sel_w].groupby(['Date', 'Session_Name', 'Session_Type']).agg({'Player Load': 'mean'}).reset_index().sort_values('Date')
            wk_trends['Day_Label'] = wk_trends['Date'].dt.strftime('%a %m/%d'); fig_tr = go.Figure(); fig_tr.add_trace(go.Scatter(x=wk_trends['Day_Label'], y=wk_trends['Player Load'], mode='lines', line=dict(color='#4895DB', width=3), showlegend=False))
    ...
    with tabs[3]:
        ...
        for i, m in enumerate(tr_metrics):
            with [t_col1, t_col2, t_col3][i]:
                fig_t = go.Figure(); p_t = tr_df[tr_df['Name'] == sel_p_pos].groupby('Week')[m].mean().reset_index(); fig_t.add_trace(go.Scatter(x=p_t['Week'], y=p_t[m], name=sel_p_pos, mode='lines+markers')); pos_t = tr_df[tr_df['Position'] == pos_label].groupby('Week')[m].mean().reset_index(); fig_t.add_trace(go.Scatter(x=pos_t['Week'], y=pos_t[m], name=f"{pos_label} Avg", line=dict(dash='dash'))); fig_t.update_layout(title=f"4-Week {m}", height=300); st.plotly_chart(fig_t, use_container_width=True, config=LOCKED_CONFIG)

    # --- TAB 4: MATCH SUMMARY (PRINT FIXES APPLIED) ---
    with tabs[4]:
        ...
        # Wrap comparison logic in print-hide div
        ...
        if selected_matches:
            ...
            match_color_map = {m: color_palette[i % 3] for i, m in enumerate(selected_matches)}
