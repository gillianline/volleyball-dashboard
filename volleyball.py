...
st.markdown("""
    ...
    /* PDF EXPORT CSS: This hides the website clutter during printing */
    @media print {
        .stTabs [role="tablist"], 
        .no-print, 
...
def load_all_data():
    ...
    df['Session_Type'] = df['Activity'].apply(lambda x: 'Match' if any(w in str(x).lower() for w in ['match', 'v.', 'vs']) else 'Practice')
...
try:
    ...
    # --- TAB 0: INDIVIDUAL PROFILE (RESTORED EXACTLY) ---
    with tabs[0]:
            ...
            with jc1:
                ...
                if not p_cmj_hist.empty:
                    ...
                    if latest['Test Date'] >= p_cmj_hist['Test Date'].max() - timedelta(days=10):
                        ...
                        label, color, profile = ("ELITE", "#28a745", "Jump Height and RSI are both High.") if cur_h >= base_h and cur_rsi >= base_rsi else ("FATIGUED", "#dc3545", "Jump Height and RSI are both Low.") if cur_h < base_h and cur_rsi < base_rsi else ("SPRINGY", "#ffc107", "Mixed Readiness")
    ...
    # --- TAB 1: TEAM GALLERY (RESTORED EXACTLY) ---
    with tabs[1]:
    ...
    # --- TAB 2: MATCH V PRACTICE (TREND FIX INCLUDED) ---
    with tabs[2]:
        ...
        with c_gg: 
            match_opts = df[(df['Name'] == gp_p) & (df['Session_Type'] == 'Match') & (df['Week'] == sel_w)]['Session_Name'].unique(); gp_g = st.selectbox("Select Specific Match", match_opts, key="gp_g_vf")
        ...
        if not w_data.empty and not g_data_l.empty:
            ...
            with cg1:
                for m in ['Total Jumps', 'Player Load', 'Estimated Distance (y)', 'Explosive Efforts']: 
                    pdif = ((w_avg[m] - g_d[m]) / g_d[m] * 100) if g_d[m] > 0 else 0; st.metric(label=m, value=f"{g_d[m]:.0f}", delta=f"{pdif:+.1f}% vs Weekly Avg")
            with cg2:
    ...
    # --- TAB 3: POSITION ANALYSIS (RESTORED EXACTLY) ---
    with tabs[3]:
    ...
    # --- TAB 4: Match SUMMARY (PDF SAFE + SAME DAY FIX) ---
    with tabs[4]:
        st.markdown('<div class="no-print">', unsafe_allow_html=True)
        if st.button("🖨️ Prepare for PDF (Hides UI)"): st.markdown('<script>window.print();</script>', unsafe_allow_html=True)
        ...
        if selected_matches:
            ...
            st.write("<br><br>", unsafe_allow_html=True); st.markdown('<div class="section-header">Team Match Averages (Sorted by Sheet Order)</div>', unsafe_allow_html=True)
            ...
            for idx, m in enumerate(t_m_list):
                with locs[idx]:
                    ...
                    fig_t.update_layout(title=f"Team Avg: {m}", showlegend=False, height=400, margin=dict(l=60, r=40, t=60, b=100), template="simple_white", bargap=0.01); st.plotly_chart(fig_t, use_container_width=True, config=LOCKED_CONFIG)
